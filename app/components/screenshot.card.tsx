    ...
    <div className={`flex flex-col gap-5 hover:bg-muted p-3 rounded-lg transition hover:cursor-pointer`}>
      <div>
        <Link passHref key={props.producthunt.id} href={`/p/${props.producthunt.id}`}>
          <img alt="" loading="lazy" className=" h-[40vh] object-cover object-top w-full rounded-t-lg border-gray-400/20 border" src={`${process.env.NEXT_PUBLIC_CLOUDFLARE_R2}/${props.producthunt.uuid}.webp` || ""}></img>
        </Link>
      </div>
      <div className="flex flex-row gap-5 items-center">
        <img alt={""} loading="lazy" className="h-10 rounded-md" src={props.producthunt.thumbnail?.url || ""}></img>
